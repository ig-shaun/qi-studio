...
import { composeRoles, CopilotOutputError, type Graph } from "@ixo-studio/core";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await composeRoles({ graph, userPrompt: "" });
    return NextResponse.json(result);
  } catch (err) {
    console.error("[api/compile/compose-roles] failed:", err);
