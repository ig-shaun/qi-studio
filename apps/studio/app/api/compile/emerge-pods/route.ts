...
import { emergePods, CopilotOutputError, type Graph } from "@ixo-studio/core";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await emergePods({ graph, userPrompt: "" });
    return NextResponse.json(result);
  } catch (err) {
    console.error("[api/compile/emerge-pods] failed:", err);
