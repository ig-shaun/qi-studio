...
import { downloadBlob, slugify } from "../io/file-utils";
...
export function MigrationPlanView({ bundle, onPlanUpdate }: Props) {
  ...
  const downloadPlan = () => {
    if (!plan) return;
    const filename = `${slugify(`${plan.sourceName}-to-${plan.targetName}`)}-migration-plan.md`;
    downloadBlob(filename, renderPlanMarkdown(plan), "text/markdown");
  };
...
const renderPlanMarkdown = (plan: MigrationPlan): string => {
  const out: string[] = [];
  out.push(`# Migration Plan`, "");
  out.push(
    `_Plan from ${plan.sourceName} -> ${plan.targetName}, generated ${formatTimestamp(
      plan.generatedAt
    )}._`,
    ""
  );
  ...
  plan.phases.forEach((phase, i) => {
    ...
    appendList(out, "Preconditions", phase.preconditions);
    appendList(out, "Key changes", phase.keyChanges);
    appendList(out, "New roles", phase.newRoles);
    appendList(out, "Retired roles", phase.retiredRoles);
    appendList(out, "Governance shifts", phase.governanceShifts);
    appendList(out, "Risks", phase.risks);
  });

  appendList(out, "Overall risks", plan.risks, 2);
  appendList(out, "Success measures", plan.successMeasures, 2);
  return out.join("\n").trimEnd() + "\n";
};
